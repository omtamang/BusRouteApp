...
export default function UserDashboard({ darkMode }) {
  ...
  async function deleteUserById() {
    try {
      ...
      console.log("Deleting route with ID:", userToDelete.passenger_id)
      const response = await deleteUser(userToDelete.passenger_id)

      // Update the UI by removing the deleted route
      setUsers(users.filter((user) => user.passenger_id !== userToDelete.passenger_id))

      // Show success notification with route ID and name
      setNotification({
        show: true,
        message: `Route "${userToDelete.passenger_name}" (ID: ${userToDelete.passenger_name}) successfully deleted`,
        type: "success",
      })
      ...
      setNotification({
        ...
        message: userToDelete
          ? `Failed to delete route "${userToDelete.passenger_name}" (ID: ${userToDelete.passenger_id})`
                ...
                {loading ? (
                  ...
                  filteredUsers.map((user, index) => (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center mr-3">{user.passenger_name}</div>
                      </th>
                      <td
                        className={`px-4 py-3 font-medium whitespace-nowrap ${darkMode ? "text-white" : "text-gray-900"}`}
                      >
